...
/*jQuery('.ft-'+el.id).mouseover(function(){opts[el.id].pause=true;});*/jQuery('.ft-'+el.id).mouseout(function(){opts[el.id].pause=false;});jQuery('#ft-title-'+el.id).mouseover(function(){opts[el.id].pause=true;});jQuery('#ft-title-'+el.id).mouseout(function(){opts[el.id].pause=false;});clearInterval(imgInt[el.id]);imgInt[el.id]=setInterval(function(){jQuery.transition(el)},params.delay+params.stripDelay*params.strips);};jQuery.transition=function(el,direction){if(opts[el.id].pause==true)return;stripInt[el.id]=setInterval(function(){jQuery.strips(order[el.id][inc[el.id]],el)},opts[el.id].stripDelay);jQuery('#'+el.id).css({'background-image':'url('+img[el.id][imgInc[el.id]]+')'});if(typeof(direction)=="undefined")
imgInc[el.id]++;else
if(direction=='prev')
imgInc[el.id]--;else
...
inc[el.id]++;};jQuery.navigation=function(el){jQuery('#'+el.id).append("<a href='#' id='ft-prev-"+el.id+"' class='ft-prev'>prev</a>");jQuery('#'+el.id).append("<a href='#' id='ft-next-"+el.id+"' class='ft-next'>next</a>");jQuery('#ft-prev-'+el.id).css({'position':'absolute','top':params.height/2-15,'left':0,'z-index':1001,'line-height':'30px','opacity':0.7}).click(function(e){e.preventDefault();jQuery.transition(el,'prev');clearInterval(imgInt[el.id]);imgInt[el.id]=setInterval(function(){jQuery.transition(el)},params.delay+params.stripDelay*params.strips);});jQuery('#ft-next-'+el.id).css({'position':'absolute','top':params.height/2-15,'right':0,'z-index':1001,'line-height':'30px','opacity':0.7}).click(function(e){e.preventDefault();jQuery.transition(el);clearInterval(imgInt[el.id]);imgInt[el.id]=setInterval(function(){jQuery.transition(el)},params.delay+params.stripDelay*params.strips);});jQuery("<div id='ft-buttons-"+el.id+"'></div>").insertAfter(jQuery('#'+el.id));jQuery('#ft-buttons-'+el.id).css({'text-align':'left','width':opts[el.id].width});for(k=1;k<img[el.id].length+1;k++){jQuery('#ft-buttons-'+el.id).append("<a href='#' class='ft-button-"+el.id+"'>"+k+"</a>");}
...
this.each(function(){init(this);});};jQuery.fn.jqFancyTransitions.defaults={width:500,height:332,strips:10,delay:5000,stripDelay:50,titleOpacity:0.7,titleSpeed:1000,position:'alternate',direction:'fountainAlternate',effect:'',navigation:false,links:false};})(jQuery);